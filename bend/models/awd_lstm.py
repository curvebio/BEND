...
class WeightDrop(nn.Module):
    ...
    def __init__(self, module: nn.Module, weight_p: float):
        ...
        self.module.h2h._parameters["weight"] = F.dropout(
        ...
        if self.bidirectional:
            ...
            self.module.h2h_reverse._parameters["weight"] = F.dropout(
                w_rev, p=self.weight_p, training=False
            )
    ...
    def _setweights(self):
        ...
        if self.bidirectional:
            ...
            self.module.h2h_reverse._parameters["weight"] = F.dropout(
                raw_w_rev, p=self.weight_p, training=self.training
            )
...
class LSTMCell(nn.Module):
    ...
    def __init__(
        ...
        if batch_first:
            raise NotImplementedError(
                "Only (seq_len, batch_size, dim) inputs supported."
            )
    ...
    def _cell_step(self, x, hidden, tokens=None):
        ...
        if tokens != None:  # explicit check because of tensor behaviour
            idx = torch.where(tokens == self.reset_token_id)[
                0
            ]  # indices that are a reset_id token
            h[idx, :] = 0
            c[idx, :] = 0
        ...
        f = torch.sigmoid(
            preact[:, self.output_size : 2 * self.output_size]
        )  # forget gate
        g = torch.tanh(preact[:, 3 * self.output_size :])  # cell gate
        o = torch.sigmoid(
            preact[:, 2 * self.output_size : 3 * self.output_size]
        )  # output gate
    ...
    def _split_hidden_state(
        self, hidden_state: List[Tuple[torch.tensor, torch.tensor]]
    ) -> Tuple[Tuple, Tuple]:
    ...
    def forward(
        ...
        for t in range(input.size(0)):
            ...
            if input_tokens is not None:
                ...
                h_t, c_t = self._cell_step(
                    inp, (h.squeeze(0), c.squeeze(0)), previous_tokens
                )
            else:
            ...
            for t in range(input.size(0)):
                ...
                if input_tokens is not None:
                    previous_tokens_reverse = (
                        input_tokens_reverse[t - 1, :] if t > 1 else None
                    )
                    h_t_reverse, c_t_reverse = self._cell_step(
                        inp_reverse,
                        (h_reverse.squeeze(), c_reverse.squeeze()),
                        previous_tokens_reverse,
                    )
                else:
                    h_t_reverse, c_t_reverse = self._cell_step(
                        inp_reverse, (h_reverse.squeeze(), c_reverse.squeeze())
                    )
                hidden_state_reverse = (
                    h_t_reverse.unsqueeze(0),
                    c_t_reverse.unsqueeze(0),
                )
                output_list_reverse.append(h_t_reverse)
            output_reverse = torch.stack(output_list_reverse)

            output = torch.cat(
                [output, torch.flip(output_reverse, [0])], dim=-1
            )  # reverse so positions match, then concat along feature dim
...
class AWDLSTM(nn.Module):
    ...
    def __init__(self, config, is_LM: bool):
        ...
        self.dropout_prob = (
            config.dropout_prob
        )  # for consistency with original, output dropout would be more fitting
        ...
        # setup LSTM cells
        if (
            self.bidirectional and self.type_2
        ):  # this is type2 biLSTM, where the outputs are concatenated between the layers. Does not work for LM.
            lstm = [
                LSTMCell(
                    config.input_size if l == 0 else config.hidden_size * 2,
                    (
                        config.hidden_size
            ...
            self.bidirectional
        ):  # type 1 bidirectionality, two separate stacks of LSTMs
            lstm = [
                LSTMCell(
                    config.input_size if l == 0 else config.hidden_size,
                    (
                        config.hidden_size
                        if l != self.num_layers - 1
                        else config.input_size
            ...
            if config.weight_dropout_prob:
                ...
                self.lstm = nn.ModuleList(lstm)
                lstm_rev = [
                    WeightDrop(layer, config.weight_dropout_prob) for layer in lstm_rev
                ]
    ...
    def forward(
        ...
        if self.bidirectional and not self.type_2:
            ...
            for i, layer in enumerate(self.lstm_rev):
                output_rev, new_hidden_state_rev = layer(
                    inputs_rev, hidden_state_rev[i], input_ids_rev
                )
            ...
            outputs_before_dropout = [
                torch.cat([fwd, rev], dim=-1)
                for fwd, rev in zip(outputs_before_dropout, outputs_before_dropout_rev)
            ]
            hidden_states = [
                (torch.cat([h, h_rev], dim=-1), torch.cat([c, c_rev], dim=-1))
                for (h, c), (h_rev, c_rev) in zip(hidden_states, hidden_states_rev)
            ]
    ...
    def _init_hidden(self, batch_size, dtype_tensor):
        ...
        states = [
            (
                weight.new_zeros(
                    1,
                    batch_size,
                    self.hidden_size if l != self.num_layers - 1 else self.input_size,
                ),
                weight.new_zeros(
                    1,
                    batch_size,
        ...
        if (
            self.bidirectional and not self.type_2
        ):  # *2 because of concatenation of forward and reverse states
            states = [
                (
                    weight.new_zeros(
                        1,
...
class AWDLSTMModel(AWDLSTMPreTrainedModel):
    def __init__(
        ...
        **kwargs,
        ...
        )  # OneHotEmbedding(config.input_size)
    ...
    def forward(self, input_ids=None, input_mask=None, hidden_state=None, **kwargs):
        if self.batch_first:
            ...
            if hidden_state is not None:
                hidden_state = [
                    (x[0].transpose(0, 1), x[1].transpose(0, 1)) for x in hidden_state
                ]
...
class AWDLSTMForLM(AWDLSTMPreTrainedModel):
    ...
    def _regularize_loss(
        ...
        if self.beta:
            ...
            if self.batch_first:
                tar = (
                    self.beta * (last_output[:, 1:] - last_output[:, :-1]).pow(2).mean()
                )
            else:
    ...
    def forward(
        self, input_ids, input_mask=None, labels=None, output_hidden_states=False
    ):
        targets = labels
        outputs = self.encoder(input_ids, input_mask, self._get_last_hidden_state())
        sequence_output, hidden_state, raw_outputs = outputs[
            :3
        ...
        if self.encoder.encoder.bidirectional:
            ...
            sequence_output, sequence_output_reverse = torch.chunk(
                sequence_output, 2, dim=-1
            )
            last_layer_raw_output, last_layer_raw_output_reverse = torch.chunk(
                last_layer_raw_output, 2, dim=-1
            )
            prediction_scores = self.decoder(sequence_output)
            prediction_scores_reverse = self.decoder(sequence_output_reverse)
            outputs = (
                torch.cat([prediction_scores, prediction_scores_reverse], dim=-1),
            )
        else:
            prediction_scores = self.decoder(sequence_output)
        ...
        if targets is not None:
            ...
            scores_fwd = (
                prediction_scores[:, :-1]
                if self.batch_first
                else prediction_scores[:-1]
            )
            loss_fct = nn.CrossEntropyLoss(
                ignore_index=-100
            )  # changed ignore_index from -1 -> -100
            raw_loss = loss_fct(
                scores_fwd.contiguous().view(-1, self.config.vocab_size),
                targets_fwd.contiguous().view(-1),
            )
            if self.encoder.encoder.bidirectional:
                targets_rev = targets[:, :-1] if self.batch_first else targets[:-1]
                scores_rev = (
                    prediction_scores_reverse[:, 1:]
                    if self.batch_first
                    else prediction_scores_reverse[1:]
                )
                raw_loss_reverse = loss_fct(
                    scores_rev.contiguous().view(-1, self.config.vocab_size),
                    targets_rev.contiguous().view(-1),
                )

                raw_loss = (raw_loss + raw_loss_reverse) / 2.0

            lm_loss = self._regularize_loss(
                raw_loss, sequence_output_full, last_layer_raw_output_full
            )
