...
class DDPTrainer:
    ...
    def __init__(
        ...
        if self.is_main_process:
            self.early_stopping_patience = getattr(
                self.config.params, "early_stopping_patience", None
            )
            self.early_stopping_min_delta = getattr(
                self.config.params, "early_stopping_min_delta", 0.0
            )
            self.early_stopping_mode = getattr(
    ...
    def _check_early_stopping(self, val_metric, epoch):
        ...
        if improved:
            ...
            print(
                f"No improvement in validation metric. Patience: {self.patience_counter}/{self.early_stopping_patience}"
            )
    ...
    def validate(self, data_loader):
        ...
        with torch.no_grad():
            for idx, (data, target) in enumerate(data_loader):
                ...
                if self.config.params.criterion == "bce":
                    ...
                    if hasattr(self.model, "module"):
                        outputs.append(
                            torch.argmax(self.model.module.softmax(output), dim=-1)
                            .detach()
                            .cpu()
                        )
                    else:
                        outputs.append(
                            torch.argmax(self.model.softmax(output), dim=-1)
                            .detach()
                            .cpu()
                        )
        ...
        if dist.is_initialized():
            ...
            if self.is_main_process:
                ...
                all_outputs = []
                all_targets = []
                for proc_outputs, proc_targets in zip(
                    gathered_outputs, gathered_targets
                ):
        ...
        if self.is_main_process:
            try:
                metrics = self._calculate_metric(
                    torch.cat(targets_all), torch.cat(outputs)
                )
            except:
                metrics = self._calculate_metric(
    ...
    def train(
        ...
        is_webdataset = "WebLoader" in train_loader.__class__.__name__ or (
            hasattr(train_loader, "dataset")
            and not hasattr(train_loader.dataset, "__len__")
        )
        ...
        for epoch in range(1 + start_epoch, epochs + 1):
            ...
            if dist.is_initialized():
                ...
                early_stop_tensor = torch.tensor(
                    early_stop, dtype=torch.bool, device=self.device
                )
        ...
        if (
            ...
            and hasattr(self, "early_stopped")
            and self.early_stopped
            ...
            if len(best_row) > 0:
                ...
                print(
                    f"Warning: Could not find epoch {self.best_epoch} in losses.csv, using default selection"
                )

        test_loss, test_metric = self.test(
            test_loader, checkpoint=test_checkpoint, overwrite=False
        )
    ...
    def test(self, test_loader, checkpoint=None, overwrite=False):
        ...
        if self.is_main_process:
            ...
            if len(metric) > 1:
                ...
                if self.config.params.metric == "mcc":
                    columns = (
                        ...
                        + [
                            f"test_recall_{n}"
                            for n in range(int((len(metric) - 1) / 2))
                        ]
                        + [
                            f"test_precision_{n}"
                            for n in range(int((len(metric) - 1) / 2))
            ...
            metrics.to_csv(
                f"{self.config.output_dir}/best_model_metrics.csv", index=False
            )
...
def ddp_worker(rank, world_size, port, cfg):
    ...
    try:
        ...
        def is_webdataset(loader):
            ...
            return (
                "WebLoader" in loader_name
                or "WebDataset" in dataset_name
                or "IterableDataset" in dataset_name
                or (
                    hasattr(loader, "dataset")
                    and not hasattr(loader.dataset, "__len__")
                )
            )
        ...
        def create_ddp_webdataset_loader(cfg, split_type, batch_size, rank, world_size):
            ...
            end_idx = (
                min((rank + 1) * files_per_process, total_files)
            ...
            if rank == 0:
                print(
                    f"DDP shard assignment: process {rank} gets files {start_idx}:{end_idx} = {len(process_files)} files"
                )
        ...
        if is_webdataset(train_loader):
            if rank == 0:
                print(
                    "Detected WebDataset/iterative dataset - using DDP with minimal workers"
                )
            ...
            if hasattr(train_loader, "num_workers"):
                print(
                    f"Setting WebDataset num_workers to 0 for DDP compatibility (was {getattr(train_loader, 'num_workers', 'unknown')})"
                )
            ...
            if (
                ...
                and hasattr(train_loader.dataset, "__len__")
            ):
            ...
            if (
                ...
                and hasattr(val_loader.dataset, "__len__")
            ):
            ...
            if (
                test_loader
                and hasattr(test_loader, "dataset")
                and hasattr(test_loader.dataset, "__len__")
            ):
...
def launch_ddp_training(cfg, train_loader=None, val_loader=None, test_loader=None):
    ...
    if world_size < 2:
        print(
            f"Warning: Only {world_size} GPU available. DDP is most effective with multiple GPUs."
        )
    ...
    for rank in range(world_size):
        p = mp.Process(target=ddp_worker, args=(rank, world_size, port, cfg))
        p.start()
        processes.append(p)
